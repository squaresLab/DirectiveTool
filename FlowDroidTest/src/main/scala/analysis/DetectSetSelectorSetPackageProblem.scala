    ...
    */
    ...
    for (cl: SootClass <- Scene.v().getClasses(SootClass.BODIES).asScala) {
      ...
      if (DetectionUtils.isCustomClassName(cl.getName())) {
        /*if (cl.getName().contains("ApplyFragment")) {
          println(cl.getName())
          println(cl.getMethods.asScala)
          for (m: SootMethod <- cl.getMethods().asScala) {
            if (m.getName().contains("NovaLauncher")) {
              println(m.getName())
              ...
              println(m.isConcrete)
              if (m.hasActiveBody && m.isConcrete) {
                println("has active body and is concrete")
